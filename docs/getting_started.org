...
#+begin_quote
Note: The default location for Homebrew installations is different for Apple Silicon Macs (like M1). To link the application to the new default Homebrew location you need to do: ~ln -s /opt/homebrew/opt/emacs-mac/Emacs.app /Applications~. For additional ways to link the app, check the [emacs-mac port documentation](https://github.com/railwaycat/homebrew-emacsmacport/wiki/Alternative-way-of-place-Emacs.app-to-Applications-directory)
#+end_quote
